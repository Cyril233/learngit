		...
		if (speed_flag_ == 0) {
			if (get_predict_rad()) {
				...
				/*predict_point.x = static_cast<int>(
					(x1 + (x2 - x1) * cos(-predict_rad * CV_PI / 180.0) - (y1 - y2) * sin(-predict_rad * CV_PI / 180.0)) / 100);
				predict_point.y = static_cast<int>(
					(y1 - (x2 - x1) * sin(-predict_rad * CV_PI / 180.0) - (y1 - y2) * cos(-predict_rad * CV_PI / 180.0)) / 100);*/
				predict_point.x = static_cast<int>(
				...
				yaw_rotation = atan(dx / fx) ;
				pitch_rotation = atan(dy / fy);
			}
			else {
