...
namespace hnurm {
	...
	static double fun(double time) {//风车角速度函数，用以计算预测角predict_rad
		/*
			...
			time单位：s
		*/
		//return   0.785 * sin(1.884 * time) + 1.305 + 3;
	...
	private:
		...
		float yaw_rotation, pitch_rotation;//云台yaw轴和pitch轴应该转到的角度（弧度制）

		double fx;//相机的y轴焦距(y轴定义为竖直轴,单位为“像素”)
		double fy;//相机的x轴焦距(x轴定义为水平轴,单位为“像素”)
